...
pub async fn run() -> Result<()> {
    ...
    let namespace = std::env::var("SAPE_NAMESPACE")
        .unwrap_or_else(|_| protocol::DEFAULT_NAMESPACE.to_string());
    ...
    match command {
        cli::Command::Relay(opt) => relay::run_relay(opt, tunnel_proto, jump_proto, &namespace).await,
