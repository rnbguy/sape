...
mod tests {
    ...
    fn tunnel_protocol_string() {
        assert_eq!(tunnel_protocol("sape").to_string(), "/sape/tunnel/1.0.0");
    }

    #[test]
    fn jump_protocol_string() {
        assert_eq!(jump_protocol("sape").to_string(), "/sape/jump/1.0.0");
    }

    #[test]
    fn relay_identify_string() {
        assert_eq!(relay_identify_protocol("sape"), "/sape-relay/0.1.0");
    }

    #[test]
    fn client_identify_string() {
        assert_eq!(client_identify_protocol("sape"), "/sape-client/0.1.0");
    }

    #[test]
    fn custom_namespace() {
        assert_eq!(tunnel_protocol("mynet").to_string(), "/mynet/tunnel/1.0.0");
    }
}
